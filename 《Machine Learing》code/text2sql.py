...
if __name__ == '__main__':
    ...
    logger.log("加载数据集耗时 %.4fs ..." % (time.time() - start_time))
    logger.log("数据集大小: train -> %d ; dev -> %d" % (len(train_dataset), len(dev_dataset)))
    ...
    if args.read_model_path and args.read_model_path != 'none':
        ...
        logger.log("加载保存的模型: %s" % (args.read_model_path))
    else:
        ...
        if args.plm is None:
            ...
            logger.log("初始化模型和词嵌入层%.2f" % (ratio))
    ...
    if args.training:
        ...
        for i in range(start_epoch, args.max_epoch):
            ...
            if dev_acc > best_result['dev_acc']:
                ...
                logger.log('最优模型: \tEpoch: %d\tDev acc: %.4f' % (i, dev_acc))

        logger.log('最佳结果: \tEpoch: %d\tDev acc: %.4f' % (best_result['iter'], best_result['dev_acc']))
    ...
    if args.testing:
        ...
        logger.log("评估耗时 %.2fs ; Dev dataset exact match/checker/beam acc：%.4f/%.4f ." % (time.time() - start_time, dev_acc, dev_acc_checker))
