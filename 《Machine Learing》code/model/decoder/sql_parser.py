...
class SqlParser(nn.Module):
    def __init__(self, args, transition_system):
        ...
        self.table_lstm_input = self.column_lstm_input
    ...
    def score(self, encodings, mask, h0, batch):
        ...
        for t in range(batch.max_action_num):
            ...
            if args.sep_cxt:
                ...
                att_vec = self.att_vec_linear(torch.cat([out, context, schema_context], dim=-1))
            else:
                att_vec = self.att_vec_linear(torch.cat([out, context], dim=-1))

            apply_rule_logprob = F.log_softmax(self.apply_rule(self.apply_rule_affine(att_vec)), dim=-1)
