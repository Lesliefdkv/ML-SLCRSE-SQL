...
def src_sum_edge_mul_dst2(src_field, dst_field, e_field, e_field2, weight_field, out_field):
    def func(edges):
        return {out_field: ((edges.src[src_field] + edges.data[e_field] + edges.data[weight_field]*edges.data[e_field2]) * edges.dst[dst_field]).sum(-1, keepdim=True)}
...
def src_sum_edge_mul_edge2(src_field, e_field1, e_field2, weight_field, score_field2, out_field):
    def func(edges):
        return {out_field: (edges.src[src_field] + edges.data[e_field1] + edges.data[weight_field]*edges.data[e_field2]) * edges.data[score_field2]}
